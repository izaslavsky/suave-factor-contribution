...
# Gate computation behind a button so filter sliders don't trigger full recomputation
run_btn = st.button("▶️ Compute Factor Contributions",
                    disabled=not (target_variable and target_value and independent_vars))

if run_btn and target_variable and target_value and independent_vars:
    with st.spinner("Computing combinations…"):
        combinations = list(product(*[df[var].dropna().unique() for var in independent_vars]))
        rows = []
        ...
        for combo in combinations:
            ...
            rule_str = " AND ".join([f"{label_lookup.get(k, k)}={v}"
            ...
            for var in independent_vars:
                ...
                red_df  = df[(df[list(reduced)] == pd.Series(reduced)).all(axis=1)] \
                          if reduced else df
                acc_red = (red_df[target_variable] == target_value).mean() \
                          if len(red_df) > 0 else None
                clean   = label_lookup.get(var, var).replace("__binned__", "")
                row[f"Contrib of {clean}"] = (
                    round(acc_full - acc_red, 2) if acc_red is not None else None
                )
            rows.append(row)

        result_df = pd.DataFrame(rows).reset_index(drop=True)
        st.session_state["fc_result"]      = result_df
        st.session_state["fc_target_val"]  = target_value

# Show results and filters only when results exist
if "fc_result" in st.session_state and st.session_state["fc_result"] is not None:
    result_df  = st.session_state["fc_result"]
    _tval      = st.session_state["fc_target_val"]
    acc_col    = f"Accuracy → {_tval}"
    contrib_cols = [c for c in result_df.columns if c.startswith("Contrib of")]

    # Filters (these only slice the already-computed result — no recomputation)
    with st.container():
    ...
    filtered_df = result_df[(result_df["n"] >= min_n) & (result_df[acc_col] >= min_acc)]
    if min_contrib > 0:
    ...
    with st.container():
        ...
        with s2:
            sort_asc = st.radio("⬆️ Order", ["Descending", "Ascending"]) == "Ascending"
